...
public class UpdateUserInfoPresenter extends BasePresenter<UpdateUserInfoContract.IView>
        implements UpdateUserInfoContract.IPresenter ,DataSource.Callback<UserCard>{
    public UpdateUserInfoPresenter(UpdateUserInfoContract.IView view) {
        super(view);
    }
    ...
    public void update(final String portraitPath, final String description, final boolean isMan) {
        start();
        final UpdateUserInfoContract.IView view = getView();
